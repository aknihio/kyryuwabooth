...
const command = new SlashCommand()
	...
	.setRun(async (client, interaction) => {
		...
		const helpEmbed = new MessageEmbed()
			...
			.setAuthor({
				name: `Команды ${ client.user.username }`,
				iconURL: client.config.iconURL,
			})
			.setTimestamp()
			.setFooter({ text: `Страница ${ pageNo + 1 } / ${ maxPages }` });
		...
		helpEmbed.addFields({
			name: "Credits",
			value: `kyryuwa booth - версия: v${
				require("../../package.json").version
			}; Build: ${ gitHash }` +
			"\n" +
			`[✨ Сервер поддержки](${ client.config.supportServer }) | [Проблемы?](${ client.config.Issues }) | []() | [Пригласить меня](https://discord.com/oauth2/authorize?client_id=${ client.config.clientId }&permissions=${ client.config.permissions }&scope=bot%20applications.commands)`,
		});
