...
const command = new SlashCommand()
	.setName("queue")
	.setDescription("показать очередь песен")
	...
	.setRun(async (client, interaction, options) => {
		...
		if (!player.queue.size || player.queue.size === 0) {
			...
			const queueEmbed = new MessageEmbed()
				.setColor(client.config.embedColor)
				.setDescription(`**♪ | Сейчас играет:** [${ title }](${ song.uri })`)
				.addFields(
					{
						name: "Длительность",
						value: song.isStream
			...
			if (player.queue.size < 11 || player.queue.totalSize < 11) {
				...
				const embedTwo = new MessageEmbed()
					.setColor(client.config.embedColor)
					.setDescription(
						`**♪ | Сейчас играет:** [${ title }](${ song.uri }) [${ player.queue.current.requester }]\n\n**Трэки в очереди**\n${ pages[page] }`,
					)
					.addFields(
						{
					...
					.setFooter({
						text: `Страница ${ page + 1 }/${ pages.length }`,
					});
				...
				const collector = interaction.channel.createMessageComponentCollector({
					filter: (b) => {
						if (b.user.id === interaction.user.id) {
							...
							return b
								.reply({
									content: `Только **${ interaction.user.tag }** может использовать эту кнопку.`,
									ephemeral: true,
								})
								.catch(() => {
								});
				...
				collector.on("collect", async (button) => {
					if (button.customId === "queue_cmd_but_1_app") {
						...
						const embedFour = new MessageEmbed()
							.setColor(client.config.embedColor)
							.setDescription(
								`**♪ | Сейчас игарет:** [${ title }](${ song.uri }) [${ player.queue.current.requester }]\n\n**Песен в очереди**\n${ pages[page] }`,
							)
							.addFields(
								{
									name: "Длительность песни",
